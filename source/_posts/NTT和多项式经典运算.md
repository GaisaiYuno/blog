...
我们假设求出了一个$B'$，满足$AB'=1\pmod{x^{\frac{n\dfrac}$，我们要求$B$，满足$AB=1 \pmod{x^n}$。其实就是一个小范围的解推出大范围的解。

那么我们有后面的部分$B-B'=0 \pmod{x^{\frac\dfrac}}}$
...
考虑现在已经求出$B'(x)$，使得$B'(x) ^2 = A(x) \pmod{x^{\frac{\dfrac}}$。

求出一个$B(x)$使得$B(x)^2=A(x)\pmod{x^{n}}$。

两边相减

$B(x)^2-B'(x)^2=0\pmod{x^{\dfrac{n}{2}}}$。

$(B(x)+B'(x))(B(x)-B'(x))=0 \pmod{x^{\dfrac{n}{2}}}$

显然$B(x)+B'(x)!=0$。

有$B(x)-B'(x)=0\pmod{x^\\dfracn}{2}}$。
...
所以$B(x)=\f\dfrac(x)+B'(x)^2}{2B'(x)}=inv(2) *(A(x)B'^{-1}(x)+B'(x))$
...
爆拆一波$C$，有$C_{i-1}^{j-1}=\fra\dfrac1)!}{(i-j)!(j-1)!}$。

两边同时除$(i-1)!$，得$\fra\dfrac)}{(i-1)!}=\fra\dfracC_i^2}}{(i-1)!}-\sum_{j=1}^{i-1} \fra\dfrac)}{(j-1)!} \times \fra\dfrac{C_{i-j}^2}}{(i-j)!} $（剩下的$(i-j)!$和$(j-1)!$刚好每人分一个）

设$A(x)=\\dfracF(i)}{(i-1)!},B(x)=\\dfrac2^{C_{i}^2}}{(i-1)!},C(x)=\\dfrac2^{C_{i}^2}}{i!}$
...
别忘了$A(x)=\f\dfracF(i)}{(i-1)!}$，所以答案要乘$(i-1)!$。
